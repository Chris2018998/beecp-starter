...
class ConnectionHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        if (Method_Statement.equals(name)) {
            return JdbcProxyUtil.createStatementProxy((Statement) re, Type_Statement, null, dsId, dsUUID);
        } else if (Method_PreparedStatement.equals(name)) {
            return JdbcProxyUtil.createStatementProxy((Statement) re, Type_PreparedStatement, (String) args[0], dsId, dsUUID);
        } else if (Method_CallableStatement.equals(name)) {
            return JdbcProxyUtil.createStatementProxy((Statement) re, Type_CallableStatement, (String) args[0], dsId, dsUUID);
        } else {
            return re;
        }
    }
